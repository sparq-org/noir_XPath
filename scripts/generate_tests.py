...
def generate_stub_functions_module(xpath_dir: Path) -> None:
    """Generate a stubs.nr module with all needed stub functions."""
    if not STUB_FUNCTIONS_NEEDED:
        # Remove stubs module if it exists but no stubs needed
    ...
    lines = [
        "//! Auto-generated stub functions for unimplemented XPath functions.",
        "//! These functions always assert false and will cause tests to fail",
        "//! until the real implementation is provided.",
        "//!",
        "//! DO NOT EDIT - This file is auto-generated by generate_tests.py",
        "",
    ]
    
    for function_name in sorted(STUB_FUNCTIONS_NEEDED):
        stub_name = get_stub_function_name(function_name)
        lines.append(f"/// Stub for {function_name} - NOT YET IMPLEMENTED")
        lines.append(f"pub fn {stub_name}() -> bool {{")
        lines.append(f'    assert(false, "{function_name} is not yet implemented");')
    ...
    print(f"  Generated stubs module with {len(STUB_FUNCTIONS_NEEDED)} stub functions")


def update_lib_nr_with_stubs(xpath_dir: Path) -> None:
    """Update lib.nr to include the stubs module if needed."""
    ...
    if STUB_FUNCTIONS_NEEDED:
        ...
        stubs_export_block = "// Re-export stub functions for unimplemented XPath functions\npub use stubs::{"
        
        # Remove old export block if present
        old_export_marker = "// Re-export stub functions for unimplemented XPath functions"
        if old_export_marker in content:
            # Find and remove the old export block
            lines = content.split('\n')
            new_lines = []
            ...
            for line in lines:
                if old_export_marker in line:
                    ...
                    continue
                if in_export_block:
                    if line.strip().startswith('pub use stubs::'):
                        # Skip all lines that are part of the stubs export
                        if '};' in line:
                            in_export_block = False
                        continue
                    elif line.strip() == '};':
